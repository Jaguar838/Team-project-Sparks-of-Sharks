...
export const pagination = (pages, page) => {
    ...
    if (page != 1) {
        paginationWrp.insertAdjacentHTML(
            'afterbegin',
            `<button value=${1} class="pagination__page-btn">&#171; ${isDefaultLanguage() ? 'First' : 'Первая'
            }</button>`,
        );
    }
    ...
    if (page != pages) {
        paginationWrp.insertAdjacentHTML(
            'beforeend',
            `<button value=${pages} class="pagination__page-btn">${isDefaultLanguage() ? 'Last' : 'Последняя'
            } &#187;</button>`,
        );
    }
};
